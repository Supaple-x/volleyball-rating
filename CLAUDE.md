...
- Детальные модальные окна для матчей/команд/игроков
- Поиск по командам, игрокам, судьям (debounce 300ms)
...
### Известные проблемы:
- **Судьи**: в списке не отображается кол-во матчей и средний рейтинг (API возвращает только id и имя)
- Dashboard "Динамика матчей" — статический SVG, не подключён к реальным данным
...
### Списки (с поиском)
- `GET /api/matches?page=1&per_page=20` - список матчей
- `GET /api/teams?search=` - команды с поиском
- `GET /api/players?page=1&per_page=50&search=` - игроки с поиском
- `GET /api/referees?search=` - судьи с поиском
...
- Модальные окна для детальной информации (матчи, игроки, команды)
